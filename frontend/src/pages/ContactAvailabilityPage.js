...
import '../assets/css/ContactAvailabilityPage.css';

function ContactAvailabilityPage() {
    //get meeting info
    ...
    const renderMeeting = (dayIndex, hour, minute, day, month, year) => {
        ...
        const meetingsInSlot = meetings.flatMap(m => {
            ...
            if (meetingYear === year && meetingMonth === month && meetingDay === day && slotTime >= meetingDate && slotTime < endTime) {
                return (
                    <div className="ContactAvil-meeting">
                        <h3>{m.title}</h3>
        ...
        if (meetingsInSlot.length > 0) {
            ...
            return (
                <td key={tdKey} className="ContactAvil-meeting ContactAvil-show" headers={`th-${dayIndex}`}>
                    {meetingsInSlot}
                </td>
            );
        } else {
            ...
            return (
                <td key={tdKey} className="ContactAvil-show" headers={`th-${dayIndex}`}></td>
            );
        }
    };
    ...
    /////////////////////////////////////////// Shuyan code ends  //////////////////////////////
        ...
        if (Array.isArray(userAvail)) {
            ...
            const NotUseavailabilityJSX = availabilityInSlot.map((avail, index) => (
                <div key={index} className={`ContactAvil-availability-${avail.availability_level}`} />
            ...
            if (NotUseavailabilityJSX.length > 0) {
                ...
                const availabilityJSX = availabilityInSlot.map((avail, index) => (
                    <td key={tdKey} className={`ContactAvil-availability-${avail.availability_level} headers={th-${dayIndex}`}></td>
    ...
    if (!meetingInfo || !meetingInfo.user) {
    ...
    return (
        <main className="ContactAvil-LinkPage">
            <div className="ContactAvil-LinkContainer">
                <div className="meeting_info">
                        <h2 className="ContactAvil-h2">{meetingInfo.title} </h2>
                        <h3 className="ContactAvil-h3"> Meeting duration: {meetingInfo.duration} </h3>
                        <h3 className="ContactAvil-h3"> Deadline to comfirm the meeting: {new Date(meetingInfo.deadline).toLocaleString('en-US', {
                                                                                            weekday: 'long', // "Monday"
                                                                                            year: 'numeric', // "2024"
                                                                                            month: 'long', // "March"
                                                                                            day: 'numeric', // "12"
                ...
                <hr className = "ContactAvil-hr" />
                <h2 className="ContactAvil-h2"> {meetingInfo.user.first_name} {meetingInfo.user.last_name}'s availability</h2>
                <div className="ContactAvil-discription"> 
                    <p className = "ContactAvil-free_time" id = "ContactAvil-prefer"> Prefered to meet </p >
                    <p className = "ContactAvil-free_time"> Less Prefered to meet</p >
                </div>

                {/* TODO:Add code for timetable rendering */}
                <div id="ContactAvil--page-container">
                    <div id="ContactAvil--content-wrap">
                        <div className="ContactAvil--main">
                            ...
                            <table>
                                ...
                                <tr>
                                    ...
                                    <th id="ContactAvil-time-title" className="ContactAvil-time-title show">Time</th>
                                    {/* Conditionally render day columns based on screen size */}
                                    {days.map((day, index) => isSmallScreen ? 
                                        (index === selectedDayIndex ? 
                                            <th key={index} className="ContactAvil-day-column show">
                                            <div className="ContactAvil-day">{day.toLocaleString('default', { weekday: 'long' })}</div>
                                            <div className="ContactAvil-date">{day.getDate()}</div>
                                            </th> : null) : 
                                        <th key={index} className="ContactAvil-day-column show">
                                            <div className="ContactAvil-day">{day.toLocaleString('default', { weekday: 'long' })}</div>
                                            <div className="ContactAvil-date">{day.getDate()}</div>
                                ...
                                <tbody id="ContactAvil-scheduleBody">
                                {Array.from({ length: 48 }, (_, index) => { // 48 slots cover 24 hours with 30-minute intervals
                                    ...
                                    return (
                                        <tr key={index}>
                                            {/* Time Column */}
                                            <td className="ContactAvil-th-time show">{`${displayHour}:${minute} ${isPM ? 'PM' : 'AM'}`}</td>
...
export default ContactAvailabilityPage;